...
LLM 기반 모델 추천은 다양한 실패 모드를 내포한다: PwC 리더보드의 논문 제목이 실제 실행 가능한 모델 ID가 아닐 수 있고, 모델마다 요구하는 입력 해상도가 다르며(224/256/384px 등), GitHub 공개 구현체가 있더라도 pretrained 가중치가 누락되거나 `torch.hub` 호환이 안 될 수 있다. 실제로 Alchemist 개발 과정에서 PwC의 "Astroformer"(실행 불가한 논문 제목)나 SwinV2의 256px 해상도 불일치 등을 경험하였다. LLM의 symbolic reasoning만으로는 이러한 실행 가능성을 판별할 수 없으며, **다중 registry 확인 + 실측 baseline 평가**로 교차 검증해야 한다.
...
네 소스의 증거를 **실행 가능성 우선**(HF Hub timm ID 보유 모델 → GitHub torch.hub 모델 → PwC 참조) 순위로 융합한다. Task 제약에 위반되는 후보를 자동 제외한 뒤, **Top-K 후보 리스트**를 Controller에 전달한다. LLM이 hallucinate한 모델명은 4-source 중 어디에도 근거가 없으므로 자연스럽게 탈락한다.

#### Controller Agent — Empirical 검증 + 실시간 감시 (Stage 2 + 전체 orchestration)

Controller는 파이프라인의 **중앙 통제자**로서, 세 가지 핵심 역할을 수행한다:

**① 추천 모델의 실행 가능성 검증.** Benchmark Agent가 추천한 모델이 실제로 로드·학습 가능한지를 다층으로 검증한다. 먼저 LLM 기반으로 제약 적합성(사전학습 소스, HW 예산, 입력 해상도)을 점검하고, timm 모델 registry 또는 GitHub `torch.hub` 호환성을 확인한다. 추천명이 어떤 registry에도 매칭되지 않으면(e.g., PwC 논문 제목 "Astroformer" → 실행 불가), 자동으로 리더보드를 탐색하여 **실행 가능한 대안 후보**로 fallback한다. 최종적으로 Top-K 후보를 **실제 GPU에서 frozen baseline 평가**하여 LLM 추론의 정확성을 실측으로 교차 검증하고, 최고 성능 후보를 Winner로 선정한다.
...
- **Vision Technique Catalog (22개 SoTA 기법):** 기법명을 train_worker가 이해하는 concrete config override로 매핑(e.g., "stochastic_depth" → `{drop_path_rate: 0.1}`, "sam_aggressive" → `{optimizer: "sam", sam_rho: 0.1}`). LLM이 제안한 텍스트와 실행 레이어 사이의 semantic gap을 해소하여, **제안된 기법이 실제 학습 코드에 정확히 반영**되게 한다.
...
```
Benchmark 추천 "Astroformer" (PwC 93.4%)
  → Controller 검증: timm 미등록 + GitHub torch.hub 미지원 → FAIL
  ...
  → Research Adapt: lr cap 적용 → 다음 trial 안정적 수렴 → 94%+ 달성
```
