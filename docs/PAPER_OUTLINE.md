...
추천명이 timm · HuggingFace · GitHub 어느 registry에도 존재하지 않거나(e.g., PwC 논문 제목 "Astroformer" → 실행 불가), 입력 해상도가 호환되지 않을 수 있다(SwinV2 256px ≠ 기본 224px). 또한 GitHub 공개 구현체가 있더라도 `torch.hub` 호환이 안 되거나 pretrained 가중치가 누락될 수 있다. LLM의 symbolic reasoning만으로는 이를 판별할 수 없으며, **다중 registry 확인 + 실측 baseline 평가**로 교차 검증해야 한다.
