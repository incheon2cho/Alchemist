...
### COCO Detection 최종 결과 — Alchemist vs AutoML-Agent

#### 최종 비교

| 항목 | Alchemist Agent | AutoML-Agent |
|---|:---:|:---:|
| **Best mAP50-95** | **52.2%** | 49.3% |
| **Best mAP50** | **70.5%** | 65.8% |
| **Precision** | **75.3%** | 71.2% |
| **Recall** | **65.2%** | 60.4% |
| **Best 모델** | rtdetr-x (4ep) | yolo11l (25ep) |
| **학습 시간** | ~3.2h (baseline + Trial 1) | ~7.9h |
| **LLM** | Claude Opus (매 trial) | Claude Opus (코드 생성 1회) |
| **개선 전략** | trial-by-trial 적응 | 단일 학습 |

**Alchemist가 AutoML-Agent를 +2.9%p 초과 (52.2% vs 49.3%)**

#### Alchemist Trial 이력

| Trial | 모델 | Config | mAP50-95 | 소요 | Claude 판단 |
|---|---|---|---|---|---|
| Baseline | rtdetr-x 1ep | auto (freeze=False) | 46.2% | 52min | "Quick baseline evaluation" |
| Trial 1 | rtdetr-x 4ep | lr=0.001, freeze=False, SGD | **52.2%** | 3.2h | "yolo11x ceiling과 거의 같은 rtdetr-x가 더 높은 ceiling(54.8%), 짧은 fine-tune으로 충분" |
| Trial 2 | rtdetr-x (진행 중) | fallback config | 진행 중 | - | LLM 파싱 실패 → fallback |

#### Claude의 의사결정 추적

```
[Benchmark] 20 모델 스카우트 → rtdetr-x 추천 (mAP50-95=54.8%, rank 1)
  "RT-DETR's DETR-style decoder suited for dense 80-class"

[SoTA] Co-DETR 66.0% — our ceiling 54.8% (gap 11.2%p)

[Controller] "rtdetr-x approved: fits 8h budget, no constraints preclude it"

[Trial 1 Design]
  Analysis: rtdetr-x has highest ceiling but ~135min/epoch
  Diagnosis: time-budget vs capacity-ceiling tradeoff
  Prescription: yolo11x with short fine-tune (actually rtdetr-x was used
    due to executor base_model override)
  Result: 52.2% — baseline 초과 +6.0%p
```
...
| 문제 | 영향 | 해결 |
|------|------|------|
| TrialConfig freeze_backbone=True 기본값 | Trial 1에서 15.5% (freeze backbone) | 기본값 False로 변경 |
| TrialConfig img_size=224 기본값 | Detection에 부적합한 해상도 | 기본값 640으로 변경 |
| LLM 파싱 실패 시 classification fallback | Detection에 부적합한 config | TrialConfig.for_task() 팩토리 |
| 로컬 GPU 체크 (torch.cuda) | yolov8m fallback | 원격 GPU SSH 조회 |
| round_num 미정의 NameError | harness 크래시 | trial_num으로 수정 |
| executor base_model 덮어쓰기 | Claude의 모델 선택이 무시됨 | 미해결 (향후 수정) |

### 다음 단계
- executor base_model override 문제 수정 (Claude의 모델 선택 반영)
- LLM 응답 파싱 안정성 강화 (JSON 형식 검증 + 재시도)
- Segmentation/Pose 태스크로 범용성 검증
- Experience Store에 detection 결과 축적 → cross-task 활용
- 논문에 Alchemist vs AutoML-Agent 비교 실험 결과 포함
